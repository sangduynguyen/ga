...
def GaussSum(x, p, n):
    return builtins.sum(gaussian(x, p[4*k], p[4*k+1], p[4*k+2]) for k in range(n))
...
def AE_IR(x,a,b,c):
    ...
    print("R2=",R2)
    # add fig1 IR
    #fig1 = plt1.figure(1, figsize=(9.5, 6.5))
    ...
    #ax3.set_xlabel('1/kT', fontsize=15)
    #ax3.set_ylabel('ln (I)', fontsize=15)
    #ax3.text(0.01, 0.1, r'y = {0:0.5f}*x+ {1:0.5f}, R2 = {2:0.5f}'
     #        .format(E_IR, b_IR, R2), transform=ax3.transAxes)
    # Plot IR
    #ax3.scatter(x_ND, y_ln, s=20, color='blue')
    #ax3.plot(x_line, y_line, '-', color='red')

    #fig1.savefig(r'GA_FOK_IR{0:0.1f}.png'.format(i))
    return -E_IR
...
ax1.text(0.01, 0.25, r'FOK model: FOM = {0:0.6f}'
